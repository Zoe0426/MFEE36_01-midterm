...
if (isset($_GET['adminName'])) {
    ...
    $totalPages = ceil($r / $perPage); #總頁數
    $rows = [];
    if ($r) { //判斷符合條件的數據總數 $totalRows 是否存在，如果存在則繼續執行，否則不執行
        if ($page > $totalPages) {
        ...
        if (isset($_GET['text']) && $_GET['text'] !== "") {
            ...
            WHERE pa.admin_name = ?
            ORDER BY `post_sid` ASC LIMIT %s, %s', ($page - 1) * $perPage, $perPage);
        }
        $rows = $pdo->prepare($sql);
        $rows->execute([$adminName]);
    }
} else {
...
<script>
    ...
    adminSelect.addEventListener('change', function() {
        ...
        location.href = 'p_readPost_api_admin.php?adminName=' + adminName;
        // if (adminName == 'Lilian') {
