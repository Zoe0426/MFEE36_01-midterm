...
$p_sql = "SELECT COUNT(1) FROM post_list_admin WHERE board_sid = 10";
...
$sql = "SELECT * FROM post_list_admin WHERE board_sid = 10";
$stmt = $pdo->query($sql);
...
if ($totalRows) { //判斷符合條件的數據總數 $totalRows 是否存在，如果存在則繼續執行，否則不執行
    ...
    $sql = sprintf("SELECT * FROM `post_list_admin` JOIN `post_board` ON `post_list_admin`.`board_sid` = `post_board`.`board_sid` WHERE `post_board`.`board_sid` = 10 LIMIT %s, %s", ($page - 1) * $perPage, $perPage); //使用 sprintf 函數生成一條 SQL 查詢語句，按照 $perPage 條記錄每次分頁查詢數據。其中 %s 是占位符，($page - 1) * $perPage 和 $perPage 是要填入的具體值。
