...
if ($totalRows) {
    ...
    if (isset($_GET['text']) && $_GET['text'] !== "") {
        $sql = "SELECT ai.`act_sid`,`type_sid`,`act_name`,`act_content`,ag.`group_date`,`group_time`,`ppl_max`,`act_post_date` FROM `act_info` ai JOIN `act_group` ag ON ai.`act_sid`=ag.`act_sid` WHERE `act_name` LIKE '%$text%' AND `type_sid` = 1 ORDER BY `act_sid` ASC";
    } else {
        $sql = sprintf("SELECT ai.`act_sid`,`type_sid`,`act_name`,`act_content`,ag.`group_date`,`group_time`,`ppl_max`,`act_post_date` FROM `act_info` ai JOIN `act_group` ag ON ai.`act_sid`=ag.`act_sid` WHERE `type_sid`=1 ORDER BY `act_sid` ASC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
    }
    ...
    <div class="row">
        ...
        <div class="mb-3 w-25">
            ...
            <select class="form-select" id="type_sid" name="type_sid" data-required="1">
                ...
                <option selected value="1">主題派對</option>
...
<script>
    ...
    aOrder.addEventListener('change', function() {
        ...
        if (ao == 2) {
            window.location.href = 'a_list_admin_TypeS_01-2.php';
        }
    });
    ...
    search.addEventListener('click', function() {
        ...
        location.href = 'a_list_admin_TypeS_01-2.php?text=' + keywordV;
    })
</script>
