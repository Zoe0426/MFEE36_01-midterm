...
if ($totalRows) {
    ...
    if (isset($_GET['text']) && $_GET['text'] !== "") {
        $sql = "SELECT ai.`act_sid`,`type_sid`,`act_name`,`act_content`,ag.`group_date`,`group_time`,`ppl_max`,`act_post_date` FROM `act_info` ai JOIN `act_group` ag ON ai.`act_sid`=ag.`act_sid` WHERE `act_name` LIKE '%$text%' AND `type_sid` = 1 ORDER BY `act_sid` DESC";
    } else {
        $sql = sprintf("SELECT ai.`act_sid`,`type_sid`,`act_name`,`act_content`,ag.`group_date`,`group_time`,`ppl_max`,`act_post_date` FROM `act_info` ai JOIN `act_group` ag ON ai.`act_sid`=ag.`act_sid` WHERE `type_sid`=1 ORDER BY `act_sid` DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
    }
...
<script>
    ...
    search.addEventListener('click', function() {
        ...
        location.href = 'a_list_admin_TypeS_01.php?text=' + keywordV;
    })
</script>
