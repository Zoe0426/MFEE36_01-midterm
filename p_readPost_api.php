...
if ($totalRows){ //判斷符合條件的數據總數 $totalRows 是否存在，如果存在則繼續執行，否則不執行
    ...
    $sql=sprintf("SELECT * FROM `post_list_admin` LIMIT %s, %s", ($page - 1)*$perPage, $perPage); //使用 sprintf 函數生成一條 SQL 查詢語句，按照 $perPage 條記錄每次分頁查詢數據。其中 %s 是占位符，($page - 1) * $perPage 和 $perPage 是要填入的具體值。
            ...
            <tbody>
                <?php foreach ($rows as $r) : ?>
                    <tr>
                        ...
                        <td><?= $r['post_date'] ?></td>
                        <td><?= $r['update_date'] ?></td>
                        <td><a href="edit.php?sid=<?= $r['post_sid'] ?>">
